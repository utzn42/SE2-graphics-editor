...
public class AggregateIterator implements Iterator {
  ...
  /**
   * Constructs an AggregateIterator with the iterator of the overlying composite.
   *
   * @param iterator Iterator variable of the overlying composite.
   */

  public AggregateIterator(Iterator iterator) {
    stack.push(iterator);
  }
  ...
  public Object next() {
    if (!isDone()) {
      ...
      //stack.push(element.createIterator());                                                       Error due to multiple inheritance issue (Aggregate/CanvasElement)
