...
public class Star extends RegularPolygon {
  ...
  private List<Coordinate> getStarCoordinates() {
    ...
    for (int i = 0; i < getEdgeAmount(); ++i) {
      float xOuter = (float) (getRadius() * Math.sin((Math.PI + (2 * i * Math.PI)) / getEdgeAmount())
          + getCenter().getX());
      float yOuter = (float) (getRadius() * Math.cos((Math.PI + (2 * i * Math.PI)) / getEdgeAmount())
          + getCenter().getY());
      coordinates.add(new Coordinate(xOuter, yOuter));
      float xInner = (float) (innerRadius * Math.sin((Math.PI + (2 * (i + 0.5) * Math.PI)) / getEdgeAmount())
          + getCenter().getX());
      float yInner = (float) (innerRadius * Math.cos((Math.PI + (2 * (i + 0.5) * Math.PI)) / getEdgeAmount())
          + getCenter().getY());
